...
# Narrative
The SignaLink by Tigertronics is a popular and convienient USB sound card for amateur radio. It has a user-configurable RJ-45 connector for interfacing with many different systems. Users change the connector pinout to suit their radio and cable. See the Tigertronics [Wiring Information](http://www.tigertronics.com/sl_wirebm.htm) and [Detailed Jumper Installation Procedure](http://www.tigertronics.com/files/signalink_jumper_procedure.pdf). 

When used with a single radio, Tigertronics' instructions are simple and effective. Users also have the option of purchasing a turnkey solution with pre-installed jumper and custom cable. 

Homebrewers or amateurs with multiple radios, however, may find themselves needing to change configurations often. Opening the SignaLink case can be inconvenient, and recalling the latest jumper configuration difficult. Further, interfacing with handheld radios - which often have a combination MIC/PTT line - adds complexity because of the extra circuit components. 

The Universal Jumper has two main purposes:
1. It includes all of the circuitry needed to interface with combined MIC/PTT handheld radios. 
2. It provides a consistent pinout for custom cable builders and homebrewers. 
...
I designed the Universal Jumper to work with my Yeasu VX-3 and uBitx. When I see an upcoming pass of the [ISS](https://www.amsat.org/amateur-radio-on-the-iss/), I can switch from WSPR on the uBitx to APRS on the VX-3 just by changing the cable. 

If indeed this is a problem that needs solving, there are certainly other ways to solve it. This jumper is a solution that works for me, and it was a fun simple project to design a build. I'm sharing it here with the hope that it helps you, too.  
...
# Design
## MIC/PTT Interface
The main element in the design is the interface to handheld radios, which have a combination MIC/PTT line, in constrast to the separate lines used by mobile and base station radios. 
...
Component values can also be adjusted to accomodate a particular radio or in general; values within the same order or magnitude should work similarly. 

## SignaLink Interface
Inside the enclosure, an 8x2 rounded pin DIP socket sits between the SignaLink circuitry and the RJ-45 external connector. 

%photo of signalink case open
...
The jumper PCB accomodates both through hole and surface mount components and provides the correct spacing to interface with the SignaLink. 

<p align="center"><img height="400" src="kicad/images/img_pcb.png"></br><b>PCB Design in KiCAD</b></p>

Gerbers are not provided as part of this project, since manufacturing requirements can vary. The PCB design is available as part of the KiCAD project, and gerbers can be exported from KiCAD. 

The PCB can also be ordered using the KiCAD project files themselves from a number of manufacturers, including OshPark, where the prototypes were made. [Click here](oshpark.com) to easily order the PCBs from OshPark. 

%picture of unpopulated PCB
...
## BOM
To make the project more useable for the community, the PCB offers a choice between surface mount and through hole. **Populate only one capacitor and one resistor.**

Reference | Quantity | Value | Package
--------- | -------: | ----- | ----
J1, J2 | 2 | 0.018-inch diameter Rounded Header Pin | 0.1-inch, 1x8-position
C1 | 0 or 1 | 0.1uF | SMD, 0805
R1 | 0 or 1 | 2.7kOhm | SMD, 0805
C2 | 0 or 1 | 0.1uF | THT Radial, 2.5mm pitch
R2 | 0 or 1 | 2.7 kOhm | THT Axial, 6.3mm max.

**Note in particular that J1 and J2 must use round pin headers - square headers will damage the SignaLink.**

The jumper is designed to work with spare parts to the extent possible. Component values and packages are common and likely to be in any homebrewer's kit. Similar values may be substituted. Always verify that the chosen values are compatible with the connected radio prior to power on. 
...
%caption to say that available parts were used, values do not exactly match schematic

## Cabling
**Custom cables are required.** Standard RJ-45 terminated CAT5 cables are unlikely to be compatible with any radio. Cables purchased from Tigertronics are also not compatible. Refer to the radio manual and the below reference for pinouts for building cables. 

# Reference
%picture of the installed jumper with rj-45 pinout labeled
